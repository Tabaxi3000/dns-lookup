...
class RecordType(StrEnum):
    ...
    SOA = "SOA"
    PTR = "PTR"
    # Security-oriented record types (Challenge 12)
    CAA = "CAA"      # RFC 8659 - which CAs may issue certs for the domain
    SSHFP = "SSHFP"  # RFC 4255 - SSH host key fingerprints
    TLSA = "TLSA"    # RFC 6698 - DANE TLS certificate association
    DNSKEY = "DNSKEY"  # RFC 4034 - DNSSEC public keys
    DS = "DS"        # RFC 4034 - DNSSEC delegation signer
...
def extract_record_value(rdata: Any,
    ...
    if record_type == RecordType.A or record_type == RecordType.AAAA:
        ...
        caa_value = (
            rdata.value.decode()
            if isinstance(rdata.value, bytes) else rdata.value
        )
        value = f'{rdata.flags} {tag} "{caa_value}"'
    else:
        # SSHFP, TLSA, DNSKEY, DS and any other type fall back to the
        # canonical presentation-format text from dnspython.
        value = rdata.to_text()
...
async def lookup(
    ...
    _query_logger.info(format_query_start(domain, record_types))
    ...
    _query_logger.info(format_query_complete(result))
