...
if TYPE_CHECKING:
    from dnslookup.analysis import DGAAnalysis, SubdomainAnalysis
    from dnslookup.enumeration import SubdomainHit
    from dnslookup.passive import PassiveRecord
...
def results_to_csv(results: list[DNSResult] | DNSResult) -> str:
    """
    Convert results to CSV with one row per record (Challenge 3)
    """
    if isinstance(results, DNSResult):
        results = [results]
    ...
    for result in results:
        query_time = round(result.query_time_ms, 2)
        if not result.records:
            writer.writerow([result.domain, "", "", "", "", query_time])
            continue
        for record in result.records:
            writer.writerow([
                result.domain,
...
def print_enum_results(domain: str, hits: list[SubdomainHit]) -> None:
    """
    Display discovered subdomains in a table (Challenge 4)
    """
    console.print()
    console.print(
        f":compass: [bold cyan]Subdomain Enumeration:[/bold cyan] "
    ...
    if not hits:
        console.print(
            Panel(
                f"[yellow]No subdomains discovered for {domain}[/yellow]",
                border_style = "yellow",
                expand = False,
            )
        )
    ...
    table.add_column("Subdomain", style = "cyan", min_width = 25)
    table.add_column("Addresses", style = "green")

    for hit in hits:
        table.add_row(hit.fqdn, ", ".join(hit.addresses))

    console.print(table)
    console.print(
        f"\n[green]:heavy_check_mark:[/green] Discovered "
        f"[bold]{len(hits)}[/bold] subdomain(s)"
    )
    console.print()
...
def print_analysis(
    domain: str,
    tunnel: SubdomainAnalysis,
    dga: DGAAnalysis,
) -> None:
    """
    Display DNS tunneling and DGA analysis (Challenges 7 and 9)
    """
    console.print()
    console.print(
        f":microscope: [bold cyan]Threat Analysis:[/bold cyan] "
        f"[bold white]{domain}[/bold white]"
    )
    console.rule(style = "blue")
    ...
    tunnel_verdict = (
        "[red]suspicious[/red]" if tunnel.suspicious else "[green]clean[/green]"
    )
    dga_verdict = (
        "[red]likely DGA[/red]" if dga.is_dga else "[green]clean[/green]"
    )
    table.add_row("Tunneling", tunnel_verdict)
    table.add_row("Subdomain entropy", f"{tunnel.entropy:.2f}")
    table.add_row("Longest label", str(tunnel.longest_label))
    table.add_row("DGA", dga_verdict)
    table.add_row("SLD entropy", f"{dga.entropy:.2f}")
    table.add_row("Consonant ratio", f"{dga.consonant_ratio:.0%}")
    table.add_row("Vowel ratio", f"{dga.vowel_ratio:.0%}")

    console.print(table)

    reasons = tunnel.reasons + dga.reasons
    if reasons:
        console.print("[yellow]Indicators:[/yellow]")
        for reason in reasons:
            console.print(f"  [red]:x:[/red] {reason}")
    else:
        console.print("[green]:heavy_check_mark:[/green] No indicators found")
    console.print()


def print_passive_records(
    records: list[PassiveRecord],
    title: str,
) -> None:
    """
    Display passive-DNS records in a table (Challenge 8)
    """
    console.print()
    console.print(f":card_index: [bold cyan]{title}[/bold cyan]")
    console.rule(style = "blue")
